...
class CartpoleEnv(mujoco_env.MujocoEnv, utils.EzPickle, MbrlEnv):
    PENDULUM_LENGTH = 0.6
    ...
    def get_rewards(self, obs, acs):
        reward = np.exp(
            -np.sum(np.square(self._get_ee_pos(obs) - np.array([0.0, CartpoleEnv.PENDULUM_LENGTH]))) /
            (CartpoleEnv.PENDULUM_LENGTH ** 2)
        )
        reward -= 0.01 * np.sum(np.square(acs))
        return reward

    def is_done(self, obs, acs):
        return np.zeros_like(obs, dtype=bool)
