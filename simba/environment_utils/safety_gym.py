...
class SafetyGymStateScorer(object):
    ...
    def cost(self, observations):
        ...
        if self.constrain_vases:
            ...
            cost += tf.cast(tf.less_equal(vases_dist, self.vases_size * 1.2), dtype=tf.float32)
        if self.constrain_hazards:
            ...
            cost += tf.cast(tf.less_equal(hazards_dist, self.hazards_size * 1.2), dtype=tf.float32)
        if self.constrain_pillars:
            ...
            cost += tf.cast(tf.less_equal(pillars_dist, self.pillars_size * 1.2), dtype=tf.float32)
        if self.constrain_gremlins:
            ...
            cost += tf.cast(tf.less_equal(gremlins_dist, self.gremlins_size * 1.2), dtype=tf.float32)
